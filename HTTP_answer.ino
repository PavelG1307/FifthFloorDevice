void handleRoot() {
  if (server.argName(0) == "active") {
    server.send(200, "text/plain", (String) ((MODEWIFI - 1) * (-1)));
  }
  if (server.argName(0) == "ssid") {
    if (MODEWIFI == 0) {
      ...
      server.arg(2).toCharArray(secret, server.arg(1).length() + 1);
      ...
      for (byte i = 0; i < 6; i++) {
        Serial.println((char)secret[i]);
        EEPROM.write(i + 32, secret[i]);
        delay(100);
      }
      EEPROM.write(0, 1);
      if (EEPROM.commit()) {
